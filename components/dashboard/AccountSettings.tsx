    ...
    <div>
      ...
      <section className="border border-gray-200 bg-white p-6 mb-6">
        ...
        <div className="space-y-6">
          <div className="flex items-start gap-4">
            <Instagram className="w-5 h-5 text-gray-400 shrink-0 mt-1" />
          ...
          <div className="flex items-start gap-4">
            <Shield className="w-5 h-5 text-gray-400 shrink-0 mt-1" />
            <div className="flex-1">
              <p className="font-mono text-[10px] tracking-widest uppercase text-gray-500 mb-1">
                Account Type
              </p>
      ...
      {artistId && (
        <section className="border-2 border-red-200 bg-red-50/50 p-5 lg:p-6">
          <h3 className="font-heading text-lg text-(--error) mb-2">
            Danger Zone
          </h3>
          ...
          <button
            ...
            className="font-mono text-xs tracking-widest uppercase px-4 py-2 bg-(--error) text-white hover:bg-red-700 transition-colors"
          >
            Delete Profile
          </button>
        </section>
      ...
      {showDeleteWarning && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-(--ink-black)/80 p-4 animate-fade-in">
          <div className="w-full max-w-md bg-white border-2 border-(--ink-black) p-8 animate-scale-in">
            <h3 className="font-heading text-2xl text-(--error) mb-4">
              Warning
            </h3>
            <p className="font-body text-(--ink-black) mb-4">
              Deleting your profile is <strong>permanent and cannot be undone</strong>. All of your:
            </p>
            <ul className="font-body text-(--gray-700) mb-6 space-y-1 ml-4">
            ...
            <p className="font-body text-(--ink-black) mb-6">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 font-mono text-xs tracking-widest uppercase px-4 py-3 bg-(--error) text-white hover:bg-red-700 transition-colors"
              >
                I Understand
              </button>
              <button
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-(--ink-black)/80 p-4 animate-fade-in">
          <div className="w-full max-w-md bg-white border-2 border-(--ink-black) p-8 animate-scale-in">
            ...
            <p className="font-body text-(--ink-black) mb-4">
              Type <span className="font-mono font-bold bg-(--gray-100) px-2 py-0.5">DELETE</span> to confirm permanent deletion:
            </p>
            ...
            <input
              ...
              className="w-full px-4 py-3 border-2 border-(--gray-300) font-mono text-lg tracking-wider focus:outline-none focus:border-(--error) mb-4"
              placeholder="Type DELETE"
              autoFocus
            />

            {deleteError && (
              <div className="border-2 border-(--error) bg-red-50 p-3 mb-4">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 font-mono text-xs tracking-widest uppercase px-4 py-3 bg-(--error) text-white hover:bg-red-700 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
              >
