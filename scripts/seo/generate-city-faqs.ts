...
async function generateCityFAQs(
  ...
  console.log(`\n📝 Generating FAQs for ${cityName}, ${state}...`);
...
Make answers specific to ${cityName}. Reference local characteristics when possible.`;
  ...
  try {
    ...
    console.error(`   ❌ Error generating FAQs: ${errorMessage}`);
    throw error;
  }
}
...
async function main() {
  ...
  console.log('🚀 City FAQ Generator (GPT-4.1-mini)\n');
  ...
  for (let i = 0; i < citiesToGenerate.length; i += BATCH_SIZE) {
    ...
    batchResults.forEach((result, index) => {
      if (result.status === 'fulfilled') {
        ...
        console.error(`   ❌ Failed for ${batch[index].name}: ${result.reason.message}`);
      }
    });
