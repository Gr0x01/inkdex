...
-- Adds the RPC function, unique index, and missing columns required by pipeline-executor.ts

-- Add missing columns for process tracking
ALTER TABLE pipeline_runs ADD COLUMN IF NOT EXISTS process_pid INTEGER;
ALTER TABLE pipeline_runs ADD COLUMN IF NOT EXISTS last_heartbeat_at TIMESTAMPTZ;

-- Index for quick PID lookups
CREATE INDEX IF NOT EXISTS idx_pipeline_runs_pid
  ON pipeline_runs (process_pid)
...
-- RPC function for atomic job creation that handles the race condition gracefully
