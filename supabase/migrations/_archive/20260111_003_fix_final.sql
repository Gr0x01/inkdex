-- Fix ambiguous column errors by dropping all function overloads first
-- Then recreate with correct signatures and column prefixes
...
DROP FUNCTION IF EXISTS find_related_artists CASCADE;

-- Step 2: Recreate search_artists_with_count with correct 7-parameter signature
CREATE FUNCTION search_artists_with_count(
...
-- Step 3: Recreate find_related_artists with fixed ambiguous column error
CREATE FUNCTION find_related_artists(
...
AS $$
DECLARE
  source_avg_embedding vector(768);  -- Renamed to avoid conflict
BEGIN
