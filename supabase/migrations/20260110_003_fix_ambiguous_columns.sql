...
-- This migration corrects issues introduced in 20260107_003_multi_location_search.sql

-- ============================================
-- Fix 1: search_artists_with_count
-- ============================================
...
BEGIN
  ...
  aggregated_artists AS (
    SELECT
      ...
      MAX(COALESCE(ri.likes_count, 0))::bigint as max_likes,  -- Cast to bigint to match return type
      jsonb_agg(
        jsonb_build_object(
  ...
  total AS (
    SELECT COUNT(DISTINCT aa.artist_id) as cnt FROM aggregated_artists aa  -- FIX: Qualified with aa alias
  )
  SELECT
    fa.id,
    fa.name,
...
COMMENT ON FUNCTION search_artists_with_count IS
  'Vector similarity search with count, Pro/Featured ranking boosts, and multi-location support. Fixed ambiguous artist_id reference.';

-- ============================================
-- Fix 2: find_related_artists
-- ============================================
...
AS $$
DECLARE
  source_embedding vector(768);  -- FIX: Renamed from avg_embedding to source_embedding to avoid collision
BEGIN
  -- Get average embedding for source artist's portfolio
  SELECT avg(embedding)::vector(768) INTO source_embedding
  ...
  SELECT
    ...
    fa.is_verified,
    fa.follower_count,
    (1 - (ae.avg_embedding <=> source_embedding))::float as similarity  -- FIX: Use source_embedding variable
  FROM filtered_artists fa
  INNER JOIN artist_embeddings ae ON ae.artist_id = fa.id
  ORDER BY ae.avg_embedding <=> source_embedding  -- FIX: Use source_embedding variable
  LIMIT match_count;
END;
$$;

COMMENT ON FUNCTION find_related_artists IS
  'Find artists with similar style using portfolio embedding comparison. Supports multi-location filtering. Fixed ambiguous avg_embedding reference.';
