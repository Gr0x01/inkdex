-- Combined search function that returns both results and total count
-- Optimizes search pagination by eliminating separate count query
...
BEGIN
  -- First, get total count of matching artists
  SELECT COUNT(DISTINCT a.id)
  INTO total_artists
  FROM artists a
  INNER JOIN portfolio_images pi ON pi.artist_id = a.id
...
COMMENT ON FUNCTION search_artists_with_count IS
  'Combined search function that returns both artist results and total count in a single query. Optimized for pagination.';
