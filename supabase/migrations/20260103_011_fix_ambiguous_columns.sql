-- Fix ambiguous column references in search functions
-- Issue: Column names in CTEs conflicting with function return types and local variables

-- ============================================
-- Fix search_artists_with_count: artist_id ambiguous
-- Line 286 referenced artist_id without table alias
-- ============================================
...
BEGIN
  ...
  aggregated_artists AS (
    SELECT
      ...
      MAX(COALESCE(ri.likes_count, 0)) as aa_max_likes,
      jsonb_agg(
        jsonb_build_object(
...
COMMENT ON FUNCTION search_artists_with_count IS
  'Vector similarity search with count and international location filtering. Fixed ambiguous column references.';

-- ============================================
-- Fix search_artists_by_embedding: same pattern for consistency
-- ============================================
...
COMMENT ON FUNCTION search_artists_by_embedding IS
  'Vector similarity search with international location filtering. Fixed ambiguous column references.';

-- ============================================
-- Fix find_related_artists: avg_embedding variable conflicts with CTE column
-- Renamed local variable to source_avg_embedding
-- ============================================
...
AS $$
DECLARE
  source_avg_embedding vector(768);  -- Renamed to avoid conflict with CTE column
BEGIN
  ...
  artist_embeddings AS (
    SELECT
      fa.id as ae_artist_id,
      avg(pi.embedding)::vector(768) as ae_avg_embedding  -- Prefixed to avoid conflicts
  ...
  FROM filtered_artists fa
  INNER JOIN artist_embeddings ae ON ae.ae_artist_id = fa.id
...
COMMENT ON FUNCTION find_related_artists IS
  'Find artists with similar style. Supports international location filtering. Fixed ambiguous column references.';
