-- Add follower_count to search_artists_by_embedding function
-- This ensures follower count is returned in search results
...
BEGIN
  ...
  WITH filtered_artists AS (
    ...
    SELECT id, name, slug, artists.city, artists.profile_image_url,
           artists.follower_count, artists.instagram_url,
...
COMMENT ON FUNCTION search_artists_by_embedding IS
  'Optimized vector similarity search for artists. Returns profile metadata including follower count. Filters by city early, uses CTEs for better query planning, and limits results efficiently.';
