-- Find related artists using vector similarity (V2 - Optimized)
-- Uses CROSS JOIN to materialize source embedding once instead of repeated subqueries
...
BEGIN
  ...
  WITH source_embedding AS (
    ...
    SELECT pi.embedding AS embed_vector
  ...
  ranked_artists AS (
    -- Find similar artists using CROSS JOIN to materialize embedding once
    SELECT
      pi.artist_id,
      MAX(1 - (pi.embedding <=> se.embed_vector)) AS max_similarity
    FROM portfolio_images pi
    CROSS JOIN source_embedding se  -- Materialize source embedding once
    ...
    WHERE pi.embedding IS NOT NULL
      ...
      AND (1 - (pi.embedding <=> se.embed_vector)) >= 0.5
...
COMMENT ON FUNCTION find_related_artists IS
  'Find related artists using vector similarity search. V2 optimized with CROSS JOIN to materialize source embedding once.';
