...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const { autoSyncEnabled } = validated.data;
    ...
    if (!artist.is_pro) {
      return NextResponse.json(
        { error: 'Pro subscription required for auto-sync' },
        { status: 403 }
      );
    }

    // 4. Update auto_sync_enabled
    ...
    console.log(`[SyncSettings] Auto-sync ${autoSyncEnabled ? 'enabled' : 'disabled'} for artist ${artist.id}`);
